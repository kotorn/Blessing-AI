...
export async function exportBasketsToGoogleSheet(
  ...
  try {
    ...
    const summaryValues = [
      ...
      ['Total Equity', portfolio?.equity || 100000, 'USDT'],
      ['Total Balance', portfolio?.balance || 100000, 'USDT'],
      ['Margin Utilization', `${portfolio?.margin_utilization || 12.4}%`, 'Healthy (<40%)'],
      ['Effective Leverage', `${portfolio?.effective_leverage || 1.15}x`, 'Within Cap (<2.0x)'],
      ['Current Drawdown', `${portfolio?.drawdown || 1.8}%`, 'Guarded (<10%)'],
      ['Risk State', portfolio?.risk_state || 'NORMAL', 'Risk Governor Active'],
      ['Aggregate Long Exp', portfolio?.long_exposure || 0.85, 'BTC / ETH'],
      ['Aggregate Short Exp', portfolio?.short_exposure || 0.40, 'Hedge / Trend'],
