...
APP_NAME="binance-cli"
APP_VERSION="2.1.1"
if [ -n "${BINANCE_CLI_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$BINANCE_CLI_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${BINANCE_CLI_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$BINANCE_CLI_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/binance/binance-cli/releases/download/v2.1.1"
elif [ -n "${BINANCE_CLI_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$BINANCE_CLI_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/binance/binance-cli/releases/download/v2.1.1"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/binance/binance-cli/releases/download/v2.1.1"
fi
if [ -n "${BINANCE_CLI_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$BINANCE_CLI_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${BINANCE_CLI_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$BINANCE_CLI_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${BINANCE_CLI_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$BINANCE_CLI_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${BINANCE_CLI_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${BINANCE_CLI_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${BINANCE_CLI_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"binance-cli","name":"binance-cli","owner":"binance","release_type":"github"},"version":"2.1.1"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/binance-cli"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/binance-cli"
fi

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
binance-cli-installer.sh

The installer for binance-cli 2.1.1

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/binance/binance-cli/releases/download/v2.1.1
...
USAGE:
    binance-cli-installer.sh [OPTIONS]
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set BINANCE_CLI_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "binance-cli-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="380f76a300d601314cf8df7751b9ec7bd3f8c57b9817c3df50432e2a36c3f400"
            _bins="binance-cli"
            _bins_js_array='"binance-cli"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            ...
            _updater_bin=""
            ;;
        "binance-cli-aarch64-unknown-linux-gnu.tar.xz")
            _arch="aarch64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="0c3786a72fe63f581c389dace7a0f8139ed9c401d0cb9453980f4dc2fa4a87ff"
            _bins="binance-cli"
            _bins_js_array='"binance-cli"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "binance-cli-x86_64-apple-darwin.tar.xz")
            _arch="x86_64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="45772f48dc00e8743b2f6f30ec3db3d3904e0862d6b8e4c10bab28f4ef91598c"
            _bins="binance-cli"
            _bins_js_array='"binance-cli"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "binance-cli-x86_64-unknown-linux-gnu.tar.xz")
            _arch="x86_64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="6b836a24f281abf590988207b0d19d4933971ca66dcf45a9e255cdc237c4deee"
            _bins="binance-cli"
            _bins_js_array='"binance-cli"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="binance-cli-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="binance-cli-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="binance-cli-aarch64-unknown-linux-gnu.tar.xz"
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="binance-cli-x86_64-unknown-linux-gnu.tar.xz"
            if ! check_glibc "2" "35"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
...
install() {
    ...
    if [ -n "${BINANCE_CLI_INSTALL_DIR:-}" ]; then
        _force_install_dir="$BINANCE_CLI_INSTALL_DIR"
